...
//    9576
    ...
    $product = wc_get_product(10559);
    echo kiotviet_get_price_html($product);
   
   
   
//   $test = tuandev_get_default_product_variation($product);
//   
//   $test = $product->get_attributes();
//   $test = new WC_Data_Store();
//   
//   $test = $product->get_default_attributes();
//   
//   $source_product = wc_get_product(10594);
//   $default_attributes = $source_product->get_attributes();
//   echo '<pre>';
...
function tuandev_process_default_product_variation($product) {
    $default_variation = tuandev_get_default_product_variation($product);
...
function kiotviet_get_price_html($product) {
...
//    $product = wc_get_product($product_id);
    ...
    if ( $product && $product->is_type( 'variable' ) && $product->has_child() ) {
        ...
        // Get default variation based on default attributes
        $change_variation = false;
        $default_variation_id = tuandev_process_default_product_variation($product);
        ...
        if ($regular_min == $max_int || $regular_max == $min_int) {
            $html_regular = "Giá gốc: " . kiotViet_formatted_price(0);
            $html = $html_regular;
            return $html;
        }
        
        if ($regular_min == $regular_max) {
            $html_regular = "Giá gốc: " . kiotViet_formatted_price($regular_max);
        } else {
            $html_regular = "Giá gốc: " . kiotViet_formatted_price($regular_min) . ' - ' . kiotViet_formatted_price($regular_max);
        }
        ...
        if ($sale_min == $max_int || $sale_max == $min_int) {
            ...
            if ($sale_min == $sale_max) {
                $html_sale = "Giá sale: " . kiotViet_formatted_price($sale_min);
            } else {
                $html_sale = "Giá sale: " . kiotViet_formatted_price($sale_min) . ' - ' . kiotViet_formatted_price($sale_max);
            }            
            $html = '<del>' . $html_regular . '</del><br/>' . $html_sale;
        }
        ...
        if ($temp_regular === '') {
            $html = "Giá gốc: " . kiotViet_formatted_price(0);
            return $html;
        } else {
            $html_regular = "Giá gốc: " . kiotViet_formatted_price($temp_regular);
        }
            
        $temp_sale = $product->get_sale_price();
        if ($temp_sale !== '') {
            $html_sale = "Giá sale: " . kiotViet_formatted_price($temp_sale);
            $html = '<del>' . $html_regular . '</del><br/>' . $html_sale;
        } else {
            $html = $html_regular;
        }
    ...
    if (empty($html)) {
            $html = "Giá gốc: " . kiotViet_formatted_price(-1); // TODO just for testing, Should be 0
    }
