...
class xoo_wsc_Cart_Data{
	...
	public function get_cart_content(){
		...
		<?php if($items): ?>
			...
			<div class="xoo-wsc-product" data-xoo_wsc="<?php echo $item['cart_key']; ?>">
				<div class="xoo-wsc-img-col">
					<?php echo $item['image']; ?>
					<a href="#" class="xoo-wsc-remove"><?php _e('Xóa',XOO_WSC_DOMAIN); ?></a>
				</div>
				<div class="xoo-wsc-sum-col">
					...
					<div class="xoo-wsc-price">
						<?php if ($item['quantity'] < 2) {?>
							<span><?php echo $item['price']; ?></span><?php
						} else {?>
						<span><?php echo $item['quantity']; ?></span> X <span><?php echo $item['price']; ?></span><?php
						}?>
					</div>
					<!-- End -->
				</div>
			...
			<div class="xoo-wsc-subtotal">
				<span><?php _e("Tổng tiền:",XOO_WSC_DOMAIN); ?></span> <?php echo $this->formatted_price(WC()->cart->subtotal); ?>
			</div>
		...
		<?php else : ?>
			...
			<span class="xoo-wsc-ecnt"><?php _e('Chưa có sản phẩm nào trong giỏ.',XOO_WSC_DOMAIN); ?></span>
