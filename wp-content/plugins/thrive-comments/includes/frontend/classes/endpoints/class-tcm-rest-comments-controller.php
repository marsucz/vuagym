...
class TCM_REST_Comments_Controller extends WP_REST_Comments_Controller {
	...
	public function get_user_comment_count( $request ) {
		...
		/**
		 * Filter for adding arguemnts when getting user comment count
		 *
	...
	public function prepare_comment_for_database( $request ) {
		...
		$comment_fields = array(
			...
			'comment_author_picture' => $request->get_param( 'comment_author_picture' ),
			...
			'upvote'                 => $request->get_param( 'upvote' ),
			'downvote'               => $request->get_param( 'downvote' ),
		);

		/**
		 * Possibility to add more fileds before a comments is registered in the database
		 *
	...
	public function rest_prepare_comment( $response, $comment, $request ) {
		...
		$response->data['comment_content']      = tcmh()->filter_comment( $response->data['content']['raw'], $comment );
		$response->data['email_hash']           = md5( $response->data['author_email'] );
