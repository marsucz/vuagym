...
if ( ! class_exists( 'YITH_WAPO_Frontend' ) ) {
     ...
     */
        ...
        public function __construct( $version ) {
            ...
            // Add meta to order
            // add_action( 'wc_add_order_item_meta', array( $this, 'order_item_meta' ), 10, 2 );
            // Deprecated
        ...
        function check_variable_product() {
            ...
            if ( $product->is_type( 'variable' ) ) {
                add_action( 'woocommerce_single_variation', array( $this, 'show_product_options' ), 15 );
            } else {
        ...
        public function printOptions( $key , $product, $type_id ,$type , $name, $value, $price ,$label='',$image='', $price_type='fixed', $description='', $required=false , $checked=false, $disabled='' , $label_position ='before' , $min = false , $max = false  ){
            ...
            $price_hmtl = ! empty( $price ) ? apply_filters( 'yith_wapo_frontend_price_html', sprintf(
                '<span class="ywapo_label_price"> + %s</span>', wc_price( $price_calculated ? $price_calculated : $price ) ) ) : '';

            $image_html = '';
            if ( $image ) {
                $image_html = '<img src="' . str_replace( 'http:', '', esc_attr( $image ) )  . '" alt="" class="ywapo_single_option_image">';
            }
            ...
            $span_label   = sprintf( '<label for="%s" class="ywapo_label ywapo_label_tag_position_%s">%s<span class="ywapo_option_label ywapo_label_position_%s">%s</span>%s</label>', $control_id , $label_position , $image_html, $label_position, $label , $required_simbol );
                ...
                foreach ( $type_list as $single_type_options ) {
                    ...
                    if ( $price >= 0 ) {
                        ...
                        if( $price > 0 && ! $this->hide_price( $product_parent_id ) ) {
                            $name .= apply_filters( 'yith_wapo_cart_item_addon_price', ' (' . wc_price( $price ) . ')' );
                        }
