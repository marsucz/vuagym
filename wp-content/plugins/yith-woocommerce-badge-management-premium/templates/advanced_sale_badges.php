...
if ( 'preview' === $product_id ) {
    ...
    if ( $product->is_type( 'variable' ) ) {
        ...
        foreach ( $children as $child_id ) {
            ...
            $saved_money_array[]     = $regular - $price;
            $current_sale_percentage = intval( round( ( $regular - $price ) / $regular * 100 ) );
            $sale_percentage_array[] = $price > 0 && $regular > 0 ? $current_sale_percentage : 0;
        }
...
$badge_info    = (object) apply_filters( 'yith_wcbm_advanced_badge_info', array(
    ...
    'labels'            => array(
        ...
        'percentage'          => '%',
        'save'                => _x( 'Tiết kiệm', 'Text in badge: preserve lenght', 'yith-woocommerce-badges-management' ),
...
if ( $badge_info->show ): ?>
    <div
        class="yith-wcbm-badge yith-wcbm-badge<?php echo $badge_info->advanced_badge_id ?> yith-wcbm-badge-advanced yith-wcbm-badge-advanced-<?php echo $badge_info->advanced_badge; ?> yith-wcbm-on-sale-badge<?php echo $badge_info->advanced_badge_id ?> <?php echo $badge_info->display_class ?>">
        ...
        <div class="yith-wcbm-badge-text">
            ...
            <?php endif; ?>
            <div class="yith-wcbm yith-wcbm-saved-money"><?php echo sprintf( $badge_info->labels[ 'amount_saved_format' ], number_format($badge_info->saved_money,0,",","."),' ' . $badge_info->currency ); ?></div>
