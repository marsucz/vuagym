...
if ( ! class_exists( 'YITH_WC_Points_Rewards_Redemption' ) ) {
	...
	class YITH_WC_Points_Rewards_Redemption {
		...
		public function get_conversion_rate_rewards() {
			$conversion = apply_filters( 'ywpar_rewards_conversion_rate', YITH_WC_Points_Rewards()->get_option( 'rewards_conversion_rate' ) );
		...
		public function calculate_rewards_discount() {
			...
			if ( $this->get_conversion_method() == 'fixed' ) {
				...
				$subtotal_cart = apply_filters( 'ywpar_rewards_points_cart_subtotal', WC()->cart->subtotal );
				if ( $subtotal_cart != 0 ) {
					$cart_discount_percentual = $this->max_discount / $subtotal_cart * 100;
					$max_points               = intval( $cart_discount_percentual / $conversion['discount'] ) * $conversion['points'];
					$cart_discount_percentual = intval( $max_points / $conversion['points'] ) * $conversion['discount'];
		...
		public function calculate_price_worth( $product_id, $points_earned ) {
			...
			if ( $product->is_type( 'variable' ) ) {
				...
				if( $variations ){
					...
					if ( count( $price_worth ) == 0 ) {
						...
						$return = wc_price( array_shift( $price_worth ) );
					} else {
			...
			if ( $conversion_method == 'fixed' ) {
				...
				$discount    = $price_from_point_earned * $point_value;
