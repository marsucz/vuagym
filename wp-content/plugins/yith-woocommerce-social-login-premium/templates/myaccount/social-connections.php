...
if( apply_filters( 'yit_social_login_show_form', true ) ) {
    ...
    <?php if (!empty($user_connections)):
        ...
        <table class="shop_table shop_table_responsive my_account_social">
            ...
            <?php foreach ($user_connections as $provider => $social): ?>
                <tr class="order">
                    <td class="sl-username"
                        data-title="<?php _e('Username', 'yith-woocommerce-social-login') ?>"><?php echo $social['button'] ?>
                    ...
                    <td class="sl-unlink"
                        data-title="<?php _e('Unlink', 'yith-woocommerce-social-login') ?>"><?php echo $social['unlink_button'] ?></td>
                </tr>
            <?php endforeach ?>
            </tbody>
    ...
    if (!empty($user_unlinked_social)):
        ...
        <a class="ywsl-social ywsl-<?php echo $key ?>"
           href="<?php echo esc_url(add_query_arg(array('ywsl_social' => $key, 'redirect' => urlencode(ywsl_curPageURL())), site_url('wp-login.php'))) ?>">
            <img src="<?php echo apply_filters('ywsl_custom_icon_' . $key, YITH_YWSL_ASSETS_URL . '/images/' . $key . '.png', $key) ?>"
