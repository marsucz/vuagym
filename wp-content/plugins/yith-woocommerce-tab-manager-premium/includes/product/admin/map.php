...
?>
<?php
  $map_info = get_post_meta($post->ID, $tab."_custom_map", true);
if ( !$map_info )
    $map_info   =   array();
    ...
    <div class="custom_tab_options">
        <div class="options_group">
            <p class="form-field">
                ...
                <input type="checkbox" name="<?php echo $tab?>_enable_width" id="custom_check_map" value="1" <?php checked( $show_w, 1 );?> />
            </p>
        </div>
        <div class="options_group">
             <p id="custom_width_enable" class="form-field" style="<?php echo $display_input; ?>">
                ...
                <input type="number" name="<?php echo $tab?>_custom_map_width" id="custom_map_width" min="100" step="1" value="<?php echo esc_attr($width);?>" />
            </p>
            <p class="form-field">
                <label for="custom_map_height"><?php _e('Height', 'yith-woocommerce-tab-manager');?></label>
                <input type="number" name="<?php echo $tab?>_custom_map_height" id="custom_map_height" min="100" step="1" value="<?php echo esc_attr($height);?>" />
            </p>
        ...
        <div class="options_group">
            <p class="form-field">
                ...
                <input type="text" name="<?php echo $tab?>_custom_map_addr" id="custom_address_map" value="<?php echo esc_attr($address);?>" />
            </p>
            <p class="form-field">
                <label for="custom_map_zoom"><?php _e('Zoom', 'yith-woocommerce-tab-manager');?></label>
                <input  type="number" id="custom_map_zoom" name="<?php echo $tab?>_custom_map_zoom" value="<?php echo esc_attr( $zoom ) ?>" min="0" max="19" step="1" />
            </p>
        </div>
    </div>
</div>
