...
if(class_exists("woocommerce")){
	class S7upf_Widget_Price_Filter extends WC_Widget {
		...
		public function __construct() {
			...
			$this->widget_description = esc_html__( 'Display a slider to filter products in your store by price.', 'kuteshop' );
			$this->widget_id          = 'woocommerce_price_filter';
			$this->widget_name        = esc_html__( 'Filter Products by Price', 'kuteshop' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'std'   => esc_html__( 'Filter by price', 'kuteshop' ),
					'label' => esc_html__( 'Title', 'kuteshop' ),
				),
			);
			...
			echo '<form method="get" action="' . esc_url( $form_action ) . '">
				<div class="price_slider_wrapper">
					...
					<div class="price_slider_amount">
						<input type="text" id="min_price" name="min_price" value="' . esc_attr( $min_price ) . '" data-min="' . esc_attr( apply_filters( 'woocommerce_price_filter_widget_min_amount', $min ) ) . '" placeholder="' . esc_attr__( 'Min price', 'kuteshop' ) . '" />
						<input type="text" id="max_price" name="max_price" value="' . esc_attr( $max_price ) . '" data-max="' . esc_attr( apply_filters( 'woocommerce_price_filter_widget_max_amount', $max ) ) . '" placeholder="' . esc_attr__( 'Max price', 'kuteshop' ) . '" />
						<button type="submit" class="button">' . esc_html__( ' Lọc ', 'kuteshop' ) . '</button>
						<div class="price_label" style="display:none;">
							' . esc_html__( 'Giá:', 'kuteshop' ) . ' <span class="from"></span> &mdash; <span class="to"></span>
						</div>
